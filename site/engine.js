import {productCompany} from './companies.js';
...
export const DEFAULTS = {currentCtcLpa:null,targetCtcLpa:25,goalDate:'',weeklyApplications:5,noticeDays:null,repoUrl:''};
...
export function payGate(job,target=25){
  const s=job.salary||{};
...
export function applyApproved(master,job,draft){
  ...
  for(const extra of draft?.extraBullets||[]){if(extra.approved&&Number.isInteger(extra.company)&&extra.company>=0&&extra.company<100&&out.experience?.[extra.company])out.experience[extra.company].bullets.push(cleanStr(extra.text,3000));}
  return out;
}
export function newWorkspace(){return {kind:'nixon-private-workspace',schemaVersion:2,settings:{...DEFAULTS},master:null,applications:{},drafts:{},versions:{}};}
...
function draftShape(d){
  ...
  requireShape(Array.isArray(d.extraBullets??[]),'extra bullets');const extraBullets=(d.extraBullets??[]).map(b=>{requireShape(object(b)&&Number.isInteger(b.company)&&b.company>=0&&b.company<100&&typeof b.text==='string'&&typeof b.approved==='boolean','extra bullet');return {company:b.company,text:b.text,approved:b.approved};});
...
export function validateWorkspace(raw){
  if(raw?.kind!=='nixon-private-workspace'||raw.schemaVersion!==2)throw new Error('Not a version 2 private workspace.');
  ...
  requireShape(typeof out.settings.goalDate==='string'&&(!out.settings.goalDate||/^\d{4}-\d{2}-\d{2}$/.test(out.settings.goalDate)&&day(out.settings.goalDate)),'goal date');
...
// Lexicographic order follows the user's priorities; fit remains an independent score.
export function priorityCompare(a,b){
 const ctc=j=>{const s=j.salary||{};return s.kind==='employer-disclosed'&&s.basis==='Annual CTC'&&s.sources?.length?(Number.isFinite(s.minLpa)?s.minLpa:Number.isFinite(s.maxLpa)?s.maxLpa:-1):-1;};
 const loc=j=>j.locationEligibility==='chennai'?2:j.locationEligibility==='remote-india'?1:0;
 return Number(Boolean(productCompany(b.company)))-Number(Boolean(productCompany(a.company)))
 ||Number(b.roleLevel==='manager')-Number(a.roleLevel==='manager')
 ||ctc(b)-ctc(a)||loc(b)-loc(a)||(scoreOf(b)??-1)-(scoreOf(a)??-1)||a.id.localeCompare(b.id);
}
