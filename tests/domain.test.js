...
test('weighted scores sum and reviewed jobs alone get scores',()=>{assert.equal(scoreOf(base),base.score.reduce((n,c)=>n+c.points,0));assert.equal(scoreOf({...base,analysisStatus:'needs-review'}),null);});
...
test('workspace defaults contain no current CTC or personal contacts',()=>{const w=newWorkspace();assert.equal(w.settings.currentCtcLpa,null);assert.equal(w.master,null);assert.equal(validateWorkspace(w).settings.targetCtcLpa,25);});
...
test("other candidate workspace cannot overwrite Nixon profile",()=>{assert.throws(()=>validateWorkspace({...newWorkspace(),kind:"sivanesh-private-workspace"}));assert.equal(newWorkspace().settings.goalDate,"");});

test('third employment section accepts an approved fact without altering master',()=>{const w=newWorkspace();w.master={...sampleMaster,experience:[...sampleMaster.experience,...sampleMaster.experience,...sampleMaster.experience]};w.drafts[base.id]={edits:{},extraBullets:[{company:2,text:'Confirmed third-role detail',approved:true}]};const v=validateWorkspace(w);const out=applyApproved(v.master,base,v.drafts[base.id]);assert.equal(out.experience[2].bullets.at(-1),'Confirmed third-role detail');assert.notEqual(w.master.experience[2].bullets.at(-1),'Confirmed third-role detail');});
test('browser persistence keys are isolated from the original candidate',()=>{const code=fs.readFileSync(new URL('../site/app.js',import.meta.url),'utf8');assert.ok(code.includes("STORE='nje-v2-private'"));assert.ok(code.includes("PUBLIC='nje-v2-imported-jobs'"));assert.ok(!code.includes('sje-v2-'));});
