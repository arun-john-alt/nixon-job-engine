...
class ScannerTests(unittest.TestCase):
    ...
    def test_relevance(self):
        self.assertTrue(scan.is_relevant('Senior QA Engineer','Chennai, India','5-7 years'))
        ...
        self.assertFalse(scan.is_relevant('QA Intern','Chennai, India',''))
        self.assertFalse(scan.is_relevant('Senior QA Engineer','Chennai, India','15-20 years'))
    def test_discovery_never_invents_match_or_salary(self):
        node={'@type':'JobPosting','title':'Senior QA Engineer','description':'<p>Supplier management, 5-7 years.</p>','hiringOrganization':{'name':'Example'},'jobLocation':{'address':{'addressLocality':'Chennai','addressCountry':'IN'}}}
    ...
    def test_salary_units(self):
        node={'title':'Senior QA Engineer','description':'Test automation','hiringOrganization':{'name':'Example'},'jobLocation':{'address':{'addressLocality':'Chennai','addressCountry':'IN'}},'baseSalary':{'currency':'INR','value':{'minValue':1200000,'maxValue':1500000,'unitText':'YEAR'}}}
        out=scan.extract(node,'https://careers.example.com/jobs/42','Example','2026-09-16')
