...
class DiscoveryTests(unittest.TestCase):
    def test_closed_index_result_is_rejected_even_if_snippet_looks_open(self):
        row={**ROW,'raw_content':'Senior QA Engineer Chennai. No longer accepting applications. Supplier management.'}
    ...
    def test_recommendation_closure_does_not_close_target(self):
        self.assertFalse(scan.closed_result({**ROW,'raw_content':'Senior QA Engineer Chennai Apply now. Similar jobs Test automation Analyst No longer accepting applications'}))
    ...
    def test_employer_without_jsonld_can_be_indexed_not_live(self):
        row={'url':'https://careers.vestas.com/job/Chennai-Operational-Buyer/1437376433','title':'Senior QA Engineer Job Details | Vestas','content':'Chennai, India. Test automation and purchase orders.'}
    ...
    def test_configured_search_budget_and_diversity(self):
        ...
        queries=scan.discovery_queries(cfg);self.assertEqual(len(queries),4);self.assertEqual(sum(li for _,li in queries),2)
        self.assertLessEqual(len(queries)+cfg['maxAvailabilityQueries'],6)
    def test_raw_search_evidence_not_published(self):
        item=scan.linkedin_lead({**ROW,'raw_content':'Senior QA Engineer Chennai Test automation. contact private-test@example.invalid'},'2026-09-18')
        self.assertNotIn('private-test',json.dumps(item))

    def test_expired_explicit_deadline_excludes_result(self):
        row={**ROW,'raw_content':'Senior QA Engineer Chennai. Job Posting End Date: 2020-08-31 Apply now'}
    ...
    def test_company_chennai_office_does_not_override_foreign_linkedin_location(self):
        row={**ROW,'title':'Example hiring Senior QA Engineer in London, United Kingdom','raw_content':'Example has offices in Chennai, India. Purchasing and supplier management.'}
